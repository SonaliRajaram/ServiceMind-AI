    ...
    try:
        ...
        if context.get("step") == "waiting_for_phone":
            ...
            if is_valid_phone(phone):
                ...
                email_sent = NotificationEmail.send_booking_confirmation(user_data)
                confirmation_msg = (
                    f"Thank you {user_data['name']}! Your request for {user_data['service']} is booked.\n"
                    f"You will receive a confirmation email at {user_data['email']} and "
                    f"an agent will contact you on {phone} shortly for further details."
                )
                if not email_sent:
                    confirmation_msg += "\n(Email could not be sent, please verify SMTP configuration.)"
        ...
        if detected_intent == "cancel":
            return "You can cancel your service by calling +91 9789735701."
